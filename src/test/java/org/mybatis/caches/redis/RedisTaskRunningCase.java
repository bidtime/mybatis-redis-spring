...
import org.mybatis.caches.utils.TaskRunning;
 ...
 */
public final class RedisTaskRunningCase extends JUnitTestBase {
    ...
    ThreadUtils.thread(ThreadComm.POOL_SIZE, new IObjectCallBack<Thread>() {
      ...
      public void callback(Thread thread) throws Exception {
        for (int j = 0; j < ThreadComm.APPLE; j++) {
          ...
          //
          TaskRunning.lockOnly(orgKey, orgKey + "-task", new IResultCallBack<Long>() {
    ...
    ThreadUtils.thread(ThreadComm.POOL_SIZE, new IObjectCallBack<Thread>() {
      ...
      public void callback(Thread thread) throws Exception {
        for (int j = 0; j < ThreadComm.APPLE; j++) {
          ...
          //
          TaskRunning.lockUnlock(orgKey, orgKey + "-task", new IResultCallBack<Long>() {
  ...
  public void insertInventoryDaily() throws Exception {
    ...
    for (int i = 0; i < all.size(); i++) {
      ...
      TaskRunning.lockOnly("insertInventoryDaily_" + orgId, "进销存台帐", new IResultCallBack<Long>() {
